...
void Shell_Sort(int fileNum)
{
	for (int i = 1; i <= fileNum; i++)
	{
		...
		vector<int> D = {929, 505, 209, 109, 41, 19, 5, 1};//Sedgewick增量序列 
		
		for (int i = 0; i < D.size(); i++)//依次选取增量序列 
		{
			for (int j = 0; j < D[i]; j++)//对D[i]个子序列进行插入排序 
			{
				for (int start = j+D[i]; start <= array.size(); start += D[i])
				{
					int tem = array[start];//摸下一张牌
					int k = start; 
					for (; (k-D[i] >= 0) && (array[k-D[i]] > tem); k -= D[i])
						array[k] = array[k-D[i]];//移位 
					array[k] = tem;//新牌落位 
				}
			}
		}
		...
		sprintf(str, "%d", i);
		string num(str);
		string fileName(::answerFolder + string("\\Shell_Sort_ans") + string(str) + string(".txt"));
...
void Radix_Sort(int fileNum)
{
	for (int i = 1; i <= fileNum; i++)
	{
		...
		const int N = 10;
		const int times = 5;//总共五位数，分配收集共五次 
		for (int i = 1; i <= times; i++)
		{
			...
			{
				char str[times+1];
				itoa(array.at(j), str, 10);//将int转为字符串
				int pos;
				if (i <= strlen(str))
					pos = str[strlen(str)-i] - '0';//字符转数字 
				else
					pos = 0;
				
				Queue[pos].push(array[j]);
			}
		...
		sprintf(str, "%d", i);
		string num(str);
		string fileName(::answerFolder + string("\\Radix_Sort_ans") + string(str) + string(".txt"));
